...
public final class DataSetManifest implements Comparable<DataSetManifest> {
	...
	/**
	 * The directory name that pending/incoming RIF data sets will be pulled from in
	 * S3.
	 */
	public static final String S3_PREFIX_PENDING_DATA_SETS = "Incoming";
	/**
	 * A regex for {@link DataSetManifest} keys in S3. Provides capturing groups for
	 * the {@link DataSetManifestId} fields.
	 */
	 ...
	 * @param timestamp
	 */
	public DataSetManifest(Instant timestamp) {
		this.timestampText = (DateTimeFormatter.ISO_INSTANT.format(timestamp));
		entries = null;
	}
	...
	public static final class DataSetManifestId implements Comparable<DataSetManifestId> {
		...
		public static DataSetManifestId parseManifestIdFromS3Key(String s3ManifestKey) {
			Matcher manifestKeyMatcher = REGEX_PENDING_MANIFEST.matcher(s3ManifestKey);
